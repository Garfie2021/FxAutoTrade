...
/*
*/
DROP PROCEDURE [indi].[spWMA角度持続量収集_Min1]
GO
...
CREATE PROCEDURE [indi].[spWMA角度持続量収集_Min1]
AS
BEGIN
	--DELETE FROM [indi].[tIndicatorHistory_Min1];
	...
	begin
		...
		FETCH NEXT FROM cursor_tRateHistory_Min1 INTO @StartDate, 
			@買いWMAs2角度, @買いWMAs2角度シグマ, @買いWMAs2角度持続時間, @買いWMAs2角度持続Rate,
			@売りWMAs2角度, @売りWMAs2角度シグマ, @売りWMAs2角度持続時間, @売りWMAs2角度持続Rate;
		...
		BEGIN
			SELECT @Cnt = Count(*)
			FROM [indi].[tIndicatorHistory_Min1]
			WHERE [通貨ペアNo] = @通貨ペアNo AND [StartDate] = @StartDate;

			IF @Cnt > 0
			BEGIN
				UPDATE [indi].[tIndicatorHistory_Min1]
			...
			ELSE
			BEGIN
				INSERT INTO [indi].[tIndicatorHistory_Min1]
					([通貨ペアNo], [StartDate], 
					 [買いWMAs2角度], [買いWMAs2角度シグマ], [買いWMAs2角度持続時間], [買いWMAs2角度持続Rate],
					 [売りWMAs2角度], [売りWMAs2角度シグマ], [売りWMAs2角度持続時間], [売りWMAs2角度持続Rate])
				VALUES
