...
BEGIN
	...
	DECLARE @back_1m		smallint = -10;		-- マイナス値にしか対応していない
	*/
	...
	select
		@買いRate_始値 = (
			select top 1 買いRate from [rate].[tRateHistory]
			where 通貨ペアNo = a.通貨ペアNo and  @StartDate <= [StartDate] and [StartDate] < @EndDate
			order by [StartDate]
		),
		@買いRate_高値 = MAX(買いRate),
		@買いRate_安値 = MIN(買いRate),
		@買いRate_終値 = (
			select top 1 買いRate from [rate].[tRateHistory]
			where 通貨ペアNo = a.通貨ペアNo and  @StartDate <= [StartDate] and [StartDate] < @EndDate
		...
		@売りRate_始値 = (
			select top 1 売りRate from [rate].[tRateHistory]
			where 通貨ペアNo = a.通貨ペアNo and  @StartDate <= [StartDate] and [StartDate] < @EndDate
			order by [StartDate]
		),
		@売りRate_高値 = MAX(売りRate),
		@売りRate_安値 = MIN(売りRate),
		@売りRate_終値 = (
			select top 1 売りRate from [rate].[tRateHistory]
			where 通貨ペアNo = a.通貨ペアNo and  @StartDate <= [StartDate] and [StartDate] < @EndDate
			order by [StartDate] desc
