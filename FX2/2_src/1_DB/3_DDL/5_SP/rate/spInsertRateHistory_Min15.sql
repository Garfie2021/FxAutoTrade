...
DROP PROCEDURE [rate].[spInsertRateHistory_Min15]
GO
...
CREATE PROCEDURE [rate].[spInsertRateHistory_Min15]
	@通貨ペアNo		tinyint = 12,
	@StartDate		datetime
AS
BEGIN
	...
	*/
	...
	select
		@買いRate_始値 = (
			select top 1 買いRate_始値 from [rate].[tRateHistory_Min5]
		...
		@買いRate_終値 = (
			select top 1 買いRate_終値 from [rate].[tRateHistory_Min5]
			where 通貨ペアNo = a.通貨ペアNo and  @StartDate <= [StartDate] and [StartDate] < @EndDate
			order by [StartDate] desc
		),
		@売りRate_始値 = (
			select top 1 売りRate_始値 from [rate].[tRateHistory_Min5]
			where 通貨ペアNo = a.通貨ペアNo and  @StartDate <= [StartDate] and [StartDate] < @EndDate
			order by [StartDate]
		),
		...
		@売りRate_終値 = (
			select top 1 売りRate_終値 from [rate].[tRateHistory_Min5]
			where 通貨ペアNo = a.通貨ペアNo and  @StartDate <= [StartDate] and [StartDate] < @EndDate
			order by [StartDate] desc
		)
	from [rate].[tRateHistory_Min5] as a
	where 通貨ペアNo = @通貨ペアNo and @StartDate <= [StartDate] and [StartDate] < @EndDate
	group by 通貨ペアNo
	--order by 通貨ペア
	;
	...
	if @cnt < 1
	begin
		Insert [rate].[tRateHistory_Min15] (
			[通貨ペアNo],
			[StartDate],
			[買いRate_始値],
			[買いRate_高値],
