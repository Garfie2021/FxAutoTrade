...
namespace DB
{
	public static class rate
	{
		...
		{
			cmd = new SqlCommand("rate.SP_ScalarCnt", cn);
		...
		{
			cmd = new SqlCommand("rate.SP_UpdateRateHistory_10m", cn);
			cmd.CommandType = CommandType.StoredProcedure;
			cmd.CommandTimeout = dbo.CommandTimeout;
		...
		{
			cmd = new SqlCommand("dbo.SP_Chk直前でRate相反している", cn);
			cmd.CommandType = CommandType.StoredProcedure;
			cmd.CommandTimeout = dbo.CommandTimeout;
		...
		{
			cmd = new SqlCommand("dbo.SP_InsertRateHistory", cn);
			cmd.CommandType = CommandType.StoredProcedure;
			cmd.CommandTimeout = dbo.CommandTimeout;
