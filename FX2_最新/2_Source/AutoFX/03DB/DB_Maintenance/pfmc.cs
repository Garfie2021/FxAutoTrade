...
namespace DB_Maintenance
{
    public static class pfmc
    {
        ...
        public static DataTable SelectポジションHourly(SqlConnection cn, int 口座No, DateTime 開始日時, DateTime 終了日時)
        {
            try
            {
                cmd = new SqlCommand("pfmc.spSelectポジションHourly", cn);
        ...
        public static DataTable SelectExecLog(SqlConnection cn, int 口座No, DateTime 開始日時, DateTime 終了日時)
        {
            try
            {
                cmd = new SqlCommand("pfmc.spSelectExecLog", cn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandTimeout = DB定数.CommandTimeout;
