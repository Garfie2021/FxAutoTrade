...
namespace 定数
{
    ...
    public enum 個人法人 : byte
    {
        個人 = 0,
        法人 = 1,
    }

    public enum DemoReal : byte
    {
        Demo = 0,
        Real = 1,
    }

    public enum 有効無効 : byte
    {
        無効 = 0,
        有効 = 1,
    }

    public enum 自動開始 : byte
    {
        Manual = 0,
        Auto = 1,
    }

    public enum MasterSlave : byte
    {
        Master = 0, // Master Rate収集とWMA計算のみ
        Slave = 1,  // Slave Rate収集とWMA計算は無し。注文とポジション管理のみ。
    }

    public enum 毎朝の自動注文開始を行う : byte
    {
        しない = 0,
        する = 1,
    }
    ...
    public static class FX定数
    {
        ...
        public const byte No_EUR_SGD = 92;
        ...
        public static int OANDA注文禁止証拠金 = 100000;   // 10万円以下ではOANDAが注文を受け付けなくなる。
