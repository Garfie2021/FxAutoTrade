...
namespace Siamese
{
	public partial class FMain : Form
	{
		...
		*/
		...
		private void ログイン()
		{
			Trade_Order.ログイン(txtFXCM_URL.Text, txtConnection.Text, txtUsername.Text, txtPassword.Text);
		...
		//{
		//	int 注文Entry数 = 0;
		//	//Trade_Rate.注文を全て削除する(ref 注文Entry数);
		...
		private void Exception共通(bool Timerを停止, Exception ex)
		{
			if (Timerを停止 == true)
			{
				全Timerを停止();
			...
			string ErrlogPath = Settings.logFolder + "\\" + DateTime.Now.ToString("yyyyMMdd") + "_err.log";

			File.AppendAllText(ErrlogPath, "\r\n");
			File.AppendAllText(ErrlogPath, DateTime.Now.ToString() + "\r\n", Encoding.GetEncoding("Shift_JIS"));
			File.AppendAllText(ErrlogPath, ex.Data + "\r\n", Encoding.GetEncoding("Shift_JIS"));
			File.AppendAllText(ErrlogPath, ex.HelpLink + "\r\n", Encoding.GetEncoding("Shift_JIS"));
			File.AppendAllText(ErrlogPath, ex.InnerException + "\r\n", Encoding.GetEncoding("Shift_JIS"));
			File.AppendAllText(ErrlogPath, ex.Message + "\r\n", Encoding.GetEncoding("Shift_JIS"));
			File.AppendAllText(ErrlogPath, ex.Source + "\r\n", Encoding.GetEncoding("Shift_JIS"));
			File.AppendAllText(ErrlogPath, ex.StackTrace + "\r\n", Encoding.GetEncoding("Shift_JIS"));
			File.AppendAllText(ErrlogPath, ex.TargetSite + "\r\n", Encoding.GetEncoding("Shift_JIS"));
		...
		private void Form1_Load(object sender, EventArgs e)
		{
			try
			{
				...
				//txtログフォルダ.Text = Directory.GetCurrentDirectory() + @"\log";
				...
				this.Text = "AutoFX  " + Directory.GetCurrentDirectory() + @"\AutoFx_Form.exe  ";

				logger.execlog_write("Start" + "\r\n");
		...
		private static DateTime timer_Order_60sec_Tick_StartDate;
		private void timer_Order_60sec_Tick(object sender, EventArgs e)
		{
			try
			{
				if (txtAuto.Text != "Auto") return;

				if (txtシステム設定_Trade時間内.Text != "時間内") return;

				timer_Order_60sec_Tick_StartDate = DateTime.Now;
				...
				using (SqlConnection cn = new SqlConnection())
				{
					...
					Trade_Order.OrderV1ポジションClose(cn, timer_Order_60sec_Tick_StartDate);

					OrderD.OrderV1注文(cn, chkRate記録以降の処理をスキップ.Checked, chk期間別Rate計算以降の処理をスキップ.Checked, 
						chkポジション更新_成行_をスキップ.Checked, timer_Order_60sec_Tick_StartDate);
				}
		...
		private void btnRate口座ログイン_Click(object sender, EventArgs e)
		{
			Trade_Order.ログイン(txtFXCM_URL.Text, txtConnection.Text, txtUsername.Text, txtPassword.Text);
		}
