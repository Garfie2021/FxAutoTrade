...
namespace Siamese
{
	public static class C注文設定
	{
		...
		{
			for (byte 通貨ペアNo = 0; 通貨ペアNo < FXCMConst.通貨ペア.Length; 通貨ペアNo++)
			{
				// 初期値を設定しておく事で、Valueの型も確定させ、バグ防止にもなる //
				dgv注文設定.Rows.Add();
				dgv注文設定.Rows[通貨ペアNo].Cells[DGVClmNo注文.通貨ペア].Value = FXCMConst.通貨ペア[通貨ペアNo];
		...
		public static void 注文基本設定(int i基準注文単位, ref DataGridView dgv注文設定)
		{
			for (byte 通貨ペアNo = 0; 通貨ペアNo < FXCMConst.通貨ペア.Length; 通貨ペアNo++)
			{
				// 初期値を設定しておく事で、Valueの型も確定させ、バグ防止にもなる //
				dgv注文設定.Rows.Add();
				dgv注文設定.Rows[通貨ペアNo].Cells[C共通.DGVClmNo注文設定_通貨ペア].Value = FXCMConst.通貨ペア[通貨ペアNo];
