...
BEGIN
	...
	DECLARE @EndDate	datetime = '2017/08/18 22:35:00';
	*/
	...
	Begin
		...
		Select 
			...
			GETDATE()
		From hstr.[tMin1]
		Where [StartDate] = (SELECT MIN([StartDate]) FROM [hstr].[tMin1] where @StartDate <= [StartDate] and [StartDate] < @EndDate);
	End;
	...
	With min1 as (
		...
		from [hstr].[tMin1] as t1 join hstr.tMin5 as t2 on t1.通貨ペアNo = t2.通貨ペアNo
		where t1.StartDate = (SELECT MIN([StartDate]) FROM [hstr].[tMin1] where @StartDate <= [StartDate] and [StartDate] < @EndDate)
    )
	Update hstr.tMin5
	Set
		買いSwap = min1.買いSwap,
	...
	From min1
	where min1.通貨ペアNo = hstr.tMin5.通貨ペアNo and [hstr].[tMin5].StartDate = @StartDate;
	...
	With min1 as (
		...
		from [hstr].tMin1 as t1 join hstr.tMin5 as t2 on t1.通貨ペアNo = t2.通貨ペアNo
		where t1.StartDate = (SELECT MAX([StartDate]) FROM [hstr].tMin1 where @StartDate <= [StartDate] and [StartDate] < @EndDate)
	)
	Update hstr.tMin5
	Set
