	...
	*/
	...
	Begin
		...
		Select 
			...
			GETDATE()
		From hstr.[tHour1]
		Where [StartDate] = (SELECT MIN([StartDate]) FROM [hstr].[tHour1] where @StartDate <= [StartDate] and [StartDate] < @EndDate);
	End;
	...
	With min1 as (
		...
		from [hstr].[tHour1] as t1 join hstr.tDay1 as t2 on t1.通貨ペアNo = t2.通貨ペアNo
		where t1.StartDate = (SELECT MIN([StartDate]) FROM [hstr].[tHour1] where @StartDate <= [StartDate] and [StartDate] < @EndDate)
    )
	Update hstr.tDay1
	Set
		買いSwap = min1.買いSwap,
	...
	From min1
	where min1.通貨ペアNo = hstr.tDay1.通貨ペアNo and [hstr].tDay1.StartDate = @StartDate;
	...
	With min1 as (
		...
		from [hstr].tHour1 as t1 join hstr.tDay1 as t2 on t1.通貨ペアNo = t2.通貨ペアNo
		where t1.StartDate = (SELECT MAX([StartDate]) FROM [hstr].tHour1 where @StartDate <= [StartDate] and [StartDate] < @EndDate)
	)
	Update hstr.tDay1
	Set
