...
DROP PROCEDURE [pfmc].[spInsertExecLog_統計_通貨ペア別]
GO
...
CREATE PROCEDURE [pfmc].[spInsertExecLog_統計_通貨ペア別]
	@back_Day1 int = -1,
	@max_Day1 int = 0
AS
BEGIN
	...
	*/
	...
	begin
		...
		--DELETE FROM [pfmc].[tExecLog_統計_通貨ペア別] Where [StartDate] = @StartDate;

		INSERT INTO [pfmc].[tExecLog_統計_通貨ペア別] (
			 [口座No]
			,[StartDate]
			,[処理区分]
			,[通貨ペアNo]
		...
		group by [口座No], [処理区分], [通貨ペアNo], [処理名], [取引状況];
