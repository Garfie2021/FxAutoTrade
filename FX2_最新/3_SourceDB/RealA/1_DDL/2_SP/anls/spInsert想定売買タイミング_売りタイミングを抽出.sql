...
/*
*/
DROP PROCEDURE [anls].[spInsert想定売買タイミング_売りタイミングを抽出]
GO
...
CREATE PROCEDURE [anls].[spInsert想定売買タイミング_売りタイミングを抽出]
	@StartDate_Month1	datetime
AS
BEGIN
	/*
	DECLARE @back_Month1 int = -1;
	DECLARE @now		Datetime = GETDATE();
	DECLARE @ThisWeek1	Datetime;
	DECLARE @StartDate	datetime;
	DECLARE @EndDate	datetime;
	DECLARE @StartDate_Month1	datetime;
	EXECUTE [cmn].[spGetThisMonth1] @now, @back_Month1, @ThisWeek1 OUTPUT, @StartDate_Month1 OUTPUT, @EndDate OUTPUT;
	*/
	...
	begin
		...
		FROM [hstr].[tMonth1]
		where 通貨ペアNo = @通貨ペアNo 
			and [売りSwap] > [買いSwap] and [売りSwap] > 0 and [売りWMAs2] < [売りWMAs14] and [売りWMAs2角度] < 0
						...
						BEGIN
							...
							BEGIN
								...
								BEGIN
									...
									--print @通貨ペアNo;
									--print @Min1_StartDate;

									DELETE FROM [anls].[t想定売買タイミング]
									WHERE [通貨ペアNo]=@通貨ペアNo AND [売買判定]='S' AND [StartDateMin1]=@Min1_StartDate
