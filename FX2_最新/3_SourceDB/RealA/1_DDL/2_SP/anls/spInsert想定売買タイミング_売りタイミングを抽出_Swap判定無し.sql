...
DROP PROCEDURE [anls].[spInsert想定売買タイミング_売りタイミングを抽出_Swap判定無し]
GO
...
CREATE PROCEDURE [anls].[spInsert想定売買タイミング_売りタイミングを抽出_Swap判定無し]
...
BEGIN
	...
	begin
		...
		FROM [hstr].[tMonth1]
		where 通貨ペアNo = @通貨ペアNo 
			and [売りWMAs2] < [売りWMAs14] and [売りWMAs2角度] < 0
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								...
								FETCH NEXT FROM cursor_Min1 INTO @Min1_StartDate;

								WHILE @@FETCH_STATUS = 0
								BEGIN
								
									DELETE FROM [anls].[t想定売買タイミング_Swap判定無し]
									WHERE [通貨ペアNo]=@通貨ペアNo AND [売買判定]='S' AND [StartDateMin1]=@Min1_StartDate

									INSERT INTO [anls].[t想定売買タイミング_Swap判定無し]
										([通貨ペアNo]
										,[売買判定]
										,[StartDateMin1]
										,[StartDateMin5]
