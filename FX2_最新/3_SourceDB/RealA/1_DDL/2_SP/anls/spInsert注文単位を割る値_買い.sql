...
/*
*/
DROP PROCEDURE [anls].[spInsert注文単位を割る値_買い]
GO
...
CREATE PROCEDURE [anls].[spInsert注文単位を割る値_買い]
AS
BEGIN
	...
	begin
		...
		BEGIN
			...
			SET @Rate_Center1 = @Rate_Center2;
			SET @Rate_Center2 = @Rate_Center;
		...
		INSERT INTO [anls].[t注文単位を割る値] (
			...
			@通貨ペアNo,
			1,
			@Rate_Center2 + (@Rate_Center2 * 0.1), --最高値の+10%までは注文対象にしとく
			@Rate_Center2,
			@注文単位を割る値_最大,
			GETDATE()
		);
