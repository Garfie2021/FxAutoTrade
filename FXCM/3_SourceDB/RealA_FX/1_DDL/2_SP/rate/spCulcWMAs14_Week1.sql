...
/*
*/
DROP PROCEDURE [rate].[spCulcWMAs14_Week1]
GO
...
CREATE PROCEDURE [rate].[spCulcWMAs14_Week1]
	@通貨ペアNo	tinyint,
	@StartDate	datetime,
...
BEGIN
	...
	*/
	...
	--FROM [hltc].[tRateHistory_Week1]
	--where 通貨ペアNo = @通貨ペアNo and StartDate <= @StartDate
	--order by [StartDate] desc
	declare cursor_tRateHistory_Week1 cursor for
	SELECT TOP 14 --  Top 14 が基本値
       [StartDate], [買いRate_終値], [売りRate_終値]
	FROM [hstr].[tWeek1]
	where 通貨ペアNo = @通貨ペアNo and StartDate <= @StartDate
	...
	open cursor_tRateHistory_Week1;

	declare @日時 datetime;
	declare @買い_終値 float = 0;
	declare @売り_終値 float = 0;
	FETCH NEXT FROM cursor_tRateHistory_Week1 INTO @日時, @買い_終値, @売り_終値;
	...
	CLOSE cursor_tRateHistory_Week1;
	DEALLOCATE cursor_tRateHistory_Week1;
